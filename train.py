...
def train1(cf):
  ...
  prob_unet =ProbUNet(latent_dim=cf.latent_dim, num_channels=cf.num_channels,
						 ...
						 initializers={'w': training_utils.he_normal(),
									   'b': tf.keras.initializers.TruncatedNormal(stddev=0.001)},
  ...
  x = tf.compat.v1.placeholder(tf.float32, shape=cf.network_input_shape)
	...
	train_elbo_summary = tf.compat.v1.summary.scalar('train_elbo', elbo)
	...
	validation_summary_op = tf.compat.v1.summary.merge([val_rec_loss_summary, val_kl_summary])
...
def save_image_for_PU(data, set):
  ...
  for j in range(3):
    ...
    for i, (im, lab) in enumerate(zip(X, y)):
      ...
      data_format='NCHW'
      img_out_path = os.path.join("/home/bdp954/PMPU_tf_version2/output1/quarter", set, "cities_name", "image"+"_"+str(j)+"_"+ str(i)+"_leftImg8bit"+".npy")
      lab_out_path = os.path.join("/home/bdp954/PMPU_tf_version2/output1//quarter", set, "cities_name", "image"+"_"+str(j)+"_"+ str(i)+"_gtFine_labelIds"+".npy")
