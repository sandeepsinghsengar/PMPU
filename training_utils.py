...
#from tfv1.python.ops.init_ops import VarianceScaling
...
def ce_loss(labels, logits, n_classes, loss_mask=None, data_format='NCHW', one_hot_labels=True, name='ce_loss'):
    ...
    with tf.compat.v1.variable_scope(name):
        ...
        if data_format == 'NCHW':
            labels = tf.compat.v1.transpose(a=labels, perm=[0,2,3,1])  
            logits = tf.compat.v1.transpose(a=logits, perm=[0,2,3,1])
        elif data_format == 'NCDHW':
            labels = tf.compat.v1.transpose(a=labels, perm=[0,2,3,4,1])
            logits = tf.compat.v1.transpose(a=logits, perm=[0,2,3,4,1])

        batch_size = tf.cast(tf.shape(input=labels)[0], tf.float32)
        ...
        if loss_mask is None:
            ce_sum = tf.reduce_sum(input_tensor=ce_per_pixel) / batch_size
            ce_mean = tf.reduce_mean(input_tensor=ce_per_pixel)
        else:
            loss_mask_flat = tf.reshape(loss_mask, [-1,])
            loss_mask_flat = (1. - tf.cast(loss_mask_flat, tf.float32))
            ce_sum = tf.reduce_sum(input_tensor=loss_mask_flat * ce_per_pixel) / batch_size
            n_valid_pixels = tf.reduce_sum(input_tensor=loss_mask_flat)
            ce_mean = tf.reduce_sum(input_tensor=loss_mask_flat * ce_per_pixel) / n_valid_pixels
